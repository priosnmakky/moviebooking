...
<form 
 method="post" action="/MoviesBooking/date/addDate">
...
<form  action="/MoviesBooking/booking/check" >
<div class="city">
  <h2>Name</h2> 
  <p>${seat.getName()}</p>
  <h2>Price</h2> 
  <p>${seat.getPrice()}</p>
    <c:choose>
           	 <c:when test="${seat.getStatus()==1}">
            <td><input type="checkbox" name="check" value="${seat.getId()}"></input></td>
          </c:when>
          <c:otherwise>
          </c:otherwise>
     </c:choose>
...
<input type="submit" value="addBooking"/>
